...
import { circle, FitBoundsOptions, LatLng, latLng, Polygon, polygon, tileLayer, Map, FeatureGroup, featureGroup, DrawEvents, Control } from 'leaflet';
    ...
    this.scanSelection = this.odmService.getScan().subscribe
      (scan => { //message contains the data sent from service
        ...
        odmService.getToken('paperless', 'paperless').subscribe((data: any) => {
          ...
          var odmlayer = tileLayer("http://stgrog.ddns.net:8000/api/projects/"+scan.project_id+"/tasks/"+scan.task_id+"/orthophoto/tiles/{z}/{x}/{y}.png?jwt=" + data.token, { maxZoom: 22 });
